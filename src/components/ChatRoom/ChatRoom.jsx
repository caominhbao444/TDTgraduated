...
const ChatRoom = ({ user, id, room }) => {
  ...
  let welcome;
  useEffect(() => {
    io.emit("join", { user, room }, (error) => { //Sending the username to the backend as the user connects.
      if (error) return alert(error);
    });
    io.on("welcome", async (data, error) => {//Getting the welcome message from the backend
      await axios.get("http://localhost:1337/api/messages?filters[room][$eq]=" + room, {
        headers: {
